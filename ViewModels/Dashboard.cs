...
namespace BaseApp.ViewModels
{
    public class Dashboard : ViewModelBase
    {
        ...
        private bool _isStartCommandSend = false;

        public bool IsStartCommandSend
        {
            get { return _isStartCommandSend; }
            set { _isStartCommandSend = value;
                OnPropertyChanged(nameof(IsStartCommandSend));
            }
        }
        ...
        private async void SendStartCommand()
        {
            try
            {
                if (_connectionService != null && _connectionService.IsConnected)
                {
                    ...
                    string responses = await _connectionService.Receive();  // Await the server response
                    if (responses == "ACK")
                    {
                        _connectionService.Send(Startcommand);
                    }
                    IsStartCommandSend = true;
        ...
        private async void SendRowToServer()
        {
            ...
            if (!IsStartCommandSend)
            {
                System.Windows.MessageBox.Show("Please start the printer by pressing the start button before sending the data .", "Start Command Required", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            try
            {
        ...
        private void StoreDataInNewFile(string newFilePath)
        {
            try
            {
                ...
                if (selectedRows.Length > 0)
                {
                    ...
                    var range = newWorksheet.Cell(1, 1).InsertData(selectedDataTable).ToString();
